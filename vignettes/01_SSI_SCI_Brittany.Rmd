...
The raster map used here is extracted from the French Naturalness Map, developed by [Guetté et al.(2021)](https://uicn.fr/CartNat/CartNat_Donnees/Note_technique_m%C3%A9thodologique/Projet%20CARTNAT_note%20technique_2021.pdf). Download the zipped folder called "Donnees_cartographiques" at [this link](https://uicn.fr/CartNat/CartNat_Donnees/). Unzip it, and place the file "Layer4_FINAL.tif" in the `data/amphibiansBrittany` folder of the SynAnthrop package. We will crop this map to the Brittany region.

```{r load spatial data}

############ Go the `raster` route (slower)
ras_raw <- raster::raster(file.path(here::here(), 
                                    "data", 
                                    "amphibiansBrittany", 
                                    "Layer4_FINAL.tif"))

# Create a bounding box to crop the map to Brittany, in WGS84
brittany_extent_wgs84 <- raster::extent(-5.5, -0.8, 47.2, 48.9)
# Convert the extent to a sf object
brittany_extent_wgs84 <- sf::st_as_sf(as(brittany_extent_wgs84, "SpatialPolygons"))
sf::st_crs(brittany_extent_wgs84) <- sf::st_crs("+proj=longlat +datum=WGS84")  # Set the original CRS
# Define the target CRS
target_crs <- sf::st_crs(ras_raw)
# Reproject the SpatialPolygons object to the target CRS
brittany_extent_lcc <- sf::st_transform(brittany_extent_wgs84, target_crs)
# Crop to Brittany
ras_brittany <- raster::crop(ras_raw, brittany_extent_lcc)


########### Go the `terra` route (not working yet)
# ras_raw <- terra::rast(file.path(here::here(), 
#                                     "data", 
#                                     "amphibiansBrittany", 
#                                     "Layer4_FINAL.tif"))
# 
# # Create a bounding box to crop the map to Brittany, in WGS84
# brittany_extent_wgs84 <- terra::ext(-5.5, -0.8, 47.2, 48.9)
# # Convert it to the Lambert Conformal Conic projection
# brittany_extent_lcc <- terra::project(brittany_extent_wgs84, from =  "EPSG:4326" , to = ras_raw)
# 
# # Crop the raster to the extent of Brittany
# ras_brittany_terra <- terra::crop(ras_raw, brittany_extent_lcc)
```
...
print(paste(missingcoord, "species point(s) were eliminated because of missing coordinates"))
```
...
ras_brittany_terra <- terra::rast(ras_brittany)

ggplot() + 
  tidyterra::geom_spatraster(data = ras_brittany_terra) +
  geom_point(data = spOcc, aes(x = X, y = Y, color = Year)) +
  # geom_point(data = hullCoord, aes(x = x, y = y), col = "red") +
  # geom_point(data = hullCoord_terra, aes(x = x, y = y), col = "black") 
...
If you are in interactive mode, you will be prompted to validate the species list detected in the dataset. Make sure that there are no typos or synonyms, as each species will be considered separately for the analyses. A warning message may pop up but will not influence the process. 

```{r execute ssi function}

ssi_results <- ssi(r = ras_brittany, 
                   resolution = c(300, 500),
                   data = spOcc,  
                   sim = 100, 
                   threshold = 30)
...
head(sp_ssi <- ssi_results$speciesScores)
```
...
head(effsize_res <- ssi_results$effSizes)
``` 
...
head(points <- ssi_results$samplesList)
```
...
sub_points <- points %>% filter(Resolution == "300",
...
sub_sp_ssi <- sp_ssi %>% filter(Resolution == "100")
